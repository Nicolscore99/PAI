...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.D_hat = collections.deque(maxlen=self.deviation_matrix_max_rank)
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            # Update SWAG-diagonal statistics
            theta_bar_param_update = (n*self.theta_bar[name] + param) / (n+1)
            theta_bar_square_param_update = (n*self.theta_bar_square[name] + param**2) / (n+1)

            self.theta_bar[name] = theta_bar_param_update
            self.theta_bar_square[name] = theta_bar_square_param_update

            current_params[name] = param - theta_bar_param_update
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            print("Full SWAG update")

            if len(self.D_hat) == self.deviation_matrix_max_rank:
                # Remove the oldest weight vector
                self.D_hat.popleft()
            
            self.D_hat.append(current_params)

        self.sigma_diag = self._calculate_covariance_matrix()                  
        
        
    def _calculate_covariance_matrix(self) -> torch.Tensor:
        """
        Calculate the covariance matrix of the SWAG posterior.
        This method should return a dictionary that maps from weight name to covariance matrix.
        """
        
        sigma = self._create_weight_copy()
        for name, param in sigma.items():
            sigma[name] = self.theta_bar_square[name] - self.theta_bar[name]**2
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        # Automatically gets updated with the pretrained weights
        self.theta_bar = self._create_weight_copy()
        self.theta_bar_square =  self._create_weight_copy()
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                # Update SWAG statistics
                if epoch % self.swag_update_freq == 0:
                    self.epoch = epoch
                    self.update_swag()

            # Save the final SWAG weights (probably unnecessary, remove if leads to confusion)
            self.theta_SWA = self.theta_bar
            # Calculate the covariance matrix
            self.sigma = self._calculate_covariance_matrix()

            # D_hat is a list of weight vectors, each element of the list is a weight vector --> already created iteratively, does not need to be copied
